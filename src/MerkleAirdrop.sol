...
contract MerkleAirdrop is EIP712 {
    ...
    error MerkleAirdrop__AlradyClaimed();
    error MerkleAirdrop_InvalidSignature();

    address[] claimers;
    bytes32 private immutable i_merkleRoot;
    IERC20 private immutable i_airdropToken;
    mapping(address cleamer => bool claimed) private s_hasClaimed;
    ...
    function claim(address account, uint256 amount, bytes32[] calldata merkleProof, uint8 v, bytes32 r, bytes32 s) external {
        if(s_hasClaimed[account]){
            revert MerkleAirdrop__AlradyClaimed();
        }
    ...
    function getMessageHash(address account, uint256 amount) public view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(
                abi.encode(MESSAGE_TYPE_HASH, AirdropClaim({account: account, amount: amount}))
            )
        );
    }
    ...
    function getMercleRoot() external view returns (bytes32) {
        return i_merkleRoot;
    }
